...
class Pitches extends React.Component {
    ...
    createPitch() {
        const pitchName = this.state.newPitchName;
    ...
    deletePitch(id) {
        fetch('http://localhost:5000/pitch/delete/' + id)
            .then(resp => resp.ok)
            .then(() => {
                this.setState({
                    pitches: this.state.pitches.filter(item => {
    ...
    render() {
        ...
        let pitchCards = pitches.map((p, i) =>
            <GridItem key={i} xs={12} sm={6} md={3}>
                <PitchCard delete={this.deletePitch} name={p.name} id={p.id} />
            </GridItem>
        )
